...
# A cited source returned by the google_search_result output block
...
# search_queries / search_sources are extracted from google_search output blocks (not
# generated by the model as JSON). evidence_cited IS model-generated JSON.
class AgentTurn(BaseModel):
    speaker: str                                          # "red" | "blue"
    argument: str                                         # full natural language argument
    key_claim: str                                        # single sentence — the core point
    concession: str | None = None                         # what this agent grants the other side
    evidence_cited: list[str] = Field(default_factory=list)   # specific stats/facts the agent used
    search_queries: list[str] = Field(default_factory=list)   # populated post-parse from outputs
    search_sources: list[SearchSource] = Field(default_factory=list)


# Verdict from the Stage 2.5 facilitator — mirrors the TradingAgents facilitator pattern
class DebateFacilitatorVerdict(BaseModel):
    prevailing_side: Literal["red", "blue", "tie"]
    reasoning: str          # 2-3 sentences explaining why this side won on evidence
    decisive_argument: str  # the single argument that tipped the verdict
...
class DebateSummary(BaseModel):
    ...
    facilitator_verdict: DebateFacilitatorVerdict
